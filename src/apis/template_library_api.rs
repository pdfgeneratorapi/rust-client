...
/// struct for typed errors of method [`get_template_library`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetTemplateLibraryError {
...
/// struct for typed errors of method [`get_template_library_item`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetTemplateLibraryItemError {
    Status401(models::InlineObject21),
    Status403(models::InlineObject23),
    Status404(models::InlineObject24),
    Status422(models::InlineObject25),
...
pub async fn get_template_library(configuration: &configuration::Configuration, tags: Option<&str>) -> Result<models::GetTemplateLibrary200Response, Error<GetTemplateLibraryError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_tags = tags;

    let uri_str = format!("{}/templates/library", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetTemplateLibrary200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetTemplateLibrary200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetTemplateLibraryError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Returns the template definition for a public template identified by its `public_id`. 
pub async fn get_template_library_item(configuration: &configuration::Configuration, public_id: &str) -> Result<models::InlineObject16, Error<GetTemplateLibraryItemError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_public_id = public_id;

    let uri_str = format!("{}/templates/library/{publicId}", configuration.base_path, publicId=crate::apis::urlencode(p_public_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetTemplateLibraryItemError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
