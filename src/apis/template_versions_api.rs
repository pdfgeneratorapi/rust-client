...
/// struct for typed errors of method [`delete_template_version`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum DeleteTemplateVersionError {
...
/// struct for typed errors of method [`get_template_version`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetTemplateVersionError {
    Status401(models::InlineObject21),
    Status403(models::InlineObject23),
    Status404(models::InlineObject24),
    Status422(models::GetTemplateVersion422Response),
    Status429(models::InlineObject26),
    Status500(models::InlineObject27),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`list_template_versions`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
/// struct for typed errors of method [`promote_template_version`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum PromoteTemplateVersionError {
    Status401(models::InlineObject21),
    Status403(models::InlineObject23),
    Status404(models::InlineObject24),
    Status422(models::InlineObject25),
...
pub async fn delete_template_version(configuration: &configuration::Configuration, template_id: i32, template_version: i32) -> Result<(), Error<DeleteTemplateVersionError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_template_id = template_id;
    let p_template_version = template_version;

    let uri_str = format!("{}/templates/{templateId}/versions/{templateVersion}", configuration.base_path, templateId=p_template_id, templateVersion=p_template_version);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<DeleteTemplateVersionError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Returns the template definition of the specified version. 
pub async fn get_template_version(configuration: &configuration::Configuration, template_id: i32, template_version: i32) -> Result<models::InlineObject16, Error<GetTemplateVersionError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_template_id = template_id;
    let p_template_version = template_version;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetTemplateVersionError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Returns a paginated list of template versions for the specified template. 
pub async fn list_template_versions(configuration: &configuration::Configuration, template_id: i32, per_page: Option<i32>, page: Option<i32>) -> Result<models::TemplateVersionCollection, Error<ListTemplateVersionsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_template_id = template_id;
    let p_per_page = per_page;
    let p_page = page;

    let uri_str = format!("{}/templates/{templateId}/versions", configuration.base_path, templateId=p_template_id);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::TemplateVersionCollection`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::TemplateVersionCollection`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListTemplateVersionsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Promotes the specified template version to production. Only one version can be production at a time. 
pub async fn promote_template_version(configuration: &configuration::Configuration, template_id: i32, template_version: i32) -> Result<models::PromoteTemplateVersion200Response, Error<PromoteTemplateVersionError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_template_id = template_id;
    let p_template_version = template_version;

    let uri_str = format!("{}/templates/{templateId}/versions/{templateVersion}/promote", configuration.base_path, templateId=p_template_id, templateVersion=p_template_version);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PromoteTemplateVersion200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PromoteTemplateVersion200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<PromoteTemplateVersionError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
