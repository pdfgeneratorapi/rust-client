...
pub async fn create_workspace(configuration: &configuration::Configuration, create_workspace_request: Option<models::CreateWorkspaceRequest>) -> Result<models::InlineObject8, Error<CreateWorkspaceError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_create_workspace_request = create_workspace_request;
    ...
    req_builder = req_builder.json(&p_create_workspace_request);
...
pub async fn delete_workspace(configuration: &configuration::Configuration, workspace_identifier: &str) -> Result<(), Error<DeleteWorkspaceError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_workspace_identifier = workspace_identifier;

    let uri_str = format!("{}/workspaces/{workspaceIdentifier}", configuration.base_path, workspaceIdentifier=crate::apis::urlencode(p_workspace_identifier));
...
pub async fn get_workspace(configuration: &configuration::Configuration, workspace_identifier: &str) -> Result<models::InlineObject8, Error<GetWorkspaceError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_workspace_identifier = workspace_identifier;
...
pub async fn get_workspaces(configuration: &configuration::Configuration, page: Option<i32>, per_page: Option<i32>) -> Result<models::InlineObject5, Error<GetWorkspacesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_page = page;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetWorkspacesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
